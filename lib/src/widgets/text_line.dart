...
class _TextLineState extends State<TextLine> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.line.hasEmbed && widget.line.childCount == 1) {
      ...
      if (embedBuilder.expanded) {
        ...
        return EmbedProxy(
          embedBuilder.build(context, widget.controller, embed, widget.readOnly, false, lineStyle),
        );
      }
    }
  ...
  TextSpan _buildTextSpan(DefaultStyles defaultStyles, LinkedList<Node> nodes, TextStyle lineStyle) {
    ...
    final children =
        nodes.map((node) => _getTextSpanFromNode(defaultStyles, node, widget.line.style)).toList(growable: false);
  ...
  TextSpan _getTextSpanFromNode(DefaultStyles defaultStyles, Node node, Style lineStyle) {
    final textNode = node as leaf.Text;
    final nodeStyle = textNode.style;
    final isLink = nodeStyle.containsKey(Attribute.link.key) && nodeStyle.attributes[Attribute.link.key]!.value != null;
  ...
  TextStyle _getInlineTextStyle(
      leaf.Text textNode, DefaultStyles defaultStyles, Style nodeStyle, Style lineStyle, bool isLink) {
  ...
  void _tapLink(String? link) {
    ...
    link = link.trim();
    if (!(widget.customLinkPrefixes + linkPrefixes).any((linkPrefix) => link!.toLowerCase().startsWith(linkPrefix))) {
      link = 'https://$link';
    }
    launchUrl(link);
  }
...
class EditableTextLine extends RenderObjectWidget {
  ...
  RenderObject createRenderObject(BuildContext context) {
    ...
    return RenderEditableTextLine(line, textDirection, textSelection, enableInteractiveSelection, hasFocus,
        devicePixelRatio, _getPadding(), color, cursorCont, defaultStyles.inlineCode!);
  }
  ...
  EdgeInsetsGeometry _getPadding() {
    return EdgeInsetsDirectional.only(start: indentWidth, top: verticalSpacing.top, bottom: verticalSpacing.bottom);
  }
}
  ...
  void paint(PaintingContext context, Offset offset) {
    if (_leading != null) {
      if (textDirection == TextDirection.ltr) {
        ...
        final parentData = _leading!.parentData as BoxParentData;
        final effectiveOffset = offset + parentData.offset;
        context.paintChild(_leading!, Offset(size.width - _leading!.size.width, effectiveOffset.dy));
      }
    }
    ...
    if (_body != null) {
      ...
      if (inlineCodeStyle.backgroundColor != null) {
        for (final item in line.children) {
          if (item is! leaf.Text || !item.style.containsKey(Attribute.inlineCode.key)) {
            continue;
          }
          final textRange = TextSelection(baseOffset: item.offset, extentOffset: item.offset + item.length);
          ...
          for (final box in rects) {
            ...
            if (inlineCodeStyle.radius == null) {
              final paintRect = Rect.fromLTRB(rect.left - 2, rect.top, rect.right + 2, rect.bottom);
              context.canvas.drawRect(paintRect, paint);
            } else {
              final paintRect =
                  RRect.fromLTRBR(rect.left - 2, rect.top, rect.right + 2, rect.bottom, inlineCodeStyle.radius!);
      ...
      if (enableInteractiveSelection &&
        ...
        if (line.isEmpty && textSelection.baseOffset <= line.offset && textSelection.extentOffset > line.offset) {
          final lineHeight = preferredLineHeight(TextPosition(offset: line.offset));
          _selectedRects?.add(TextBox.fromLTRBD(0, 0, 3, lineHeight, textDirection));
        }
