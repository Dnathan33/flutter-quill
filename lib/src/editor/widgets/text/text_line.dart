...
import 'package:flutter/gestures.dart' show GestureRecognizer, LongPressGestureRecognizer, TapGestureRecognizer;
import 'package:flutter/material.dart';
import 'package:flutter/rendering.dart' show BoxParentData, PipelineOwner, BoxHitTestResult, RenderObjectVisitor;
...
class _TextLineState extends State<TextLine> {
  ...
  InlineSpan _scriptSpan(String text, bool superScript, TextStyle style, DefaultStyles defaultStyles) {
    ...
    final charStyle = style.copyWith(fontFeatures: <FontFeature>[], fontWeight: fontWeight, fontSize: fontSize * 0.7);
    //
  ...
  InlineSpan _getTextSpanFromNode(DefaultStyles defaultStyles, Node node, Style lineStyle) {
    ...
    if (widget.controller.configurations.requireScriptFontFeatures == false && textNode.value.isNotEmpty) {
    ...
    if (_body != null) {
      ...
      if (inlineCodeStyle.backgroundColor != null) {
        for (final item in line.children) {
          if (item is! leaf.QuillText || !item.style.containsKey(Attribute.inlineCode.key)) {
            continue;
          }
